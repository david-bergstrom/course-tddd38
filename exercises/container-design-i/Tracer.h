...
public:
   ...
   Tracer(const T& init) : value(init) {
      if (trace) std::cout << "TRACE: T ctor for new Tracer[" << this << "](" << value << ")\n";
   }
   ...
   Tracer(Tracer&& t) : value(std::move(t.value)) {
      if (trace) std::cout << "TRACE: move ctor for new Tracer[" << this << "](" << &t << ")\n";
   }
   ...
   Tracer& operator=(Tracer&& t) {
      ...
      if (trace) std::cout << "TRACE: move assignment for Tracer[" << this << "] = " << &t << "\n";
      return *this;
   }
